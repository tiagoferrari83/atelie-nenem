...
from constants import formatar_moeda, TIPO_MATERIAL_LABELS
...
def dialog_editar_material(material):
    ...
    tipo_material_atual = material.get("tipo_material") or "outros"
    tipo_material = st.selectbox(
        "Tipo de material",
        options=list(TIPO_MATERIAL_LABELS.keys()),
        format_func=lambda x: TIPO_MATERIAL_LABELS[x],
        index=list(TIPO_MATERIAL_LABELS.keys()).index(tipo_material_atual),
        help="Usado para organizar os itens no orçamento (seções Tecido / Aviamento / Outros).",
    )
    tipo_medida = st.selectbox(
        "Tipo de medida",
        options=list(TIPO_MEDIDA_LABELS.keys()),
        format_func=lambda x: TIPO_MEDIDA_LABELS[x],
        index=list(TIPO_MEDIDA_LABELS.keys()).index(material["tipo_medida"]),
    )
    valor = st.number_input(
        "Valor (R$)", min_value=0.0, step=0.5, format="%.2f", value=float(material["valor"])
    )
    ...
    if st.button("Salvar alterações", type="primary"):
        if not nome:
            ...
            execute(
                ...
                SET nome = %s, tipo_material = %s, tipo_medida = %s, valor = %s,
                    atualizado_em = NOW()
                WHERE id = %s
                """,
                (nome, tipo_material, tipo_medida, valor, material["id"]),
            )
...
with st.form("form_materia", clear_on_submit=True):
    ...
    if submitted:
        if not nome:
            ...
            execute(
                """
                INSERT INTO materia_prima (nome, tipo_material, tipo_medida, valor)
                VALUES (%s, %s, %s, %s)
                """,
...
if not materiais:
    ...
    # Agrupa por tipo_material para facilitar a visualização
    grupos = {"tecido": [], "aviamento": [], "outros": []}
    for m in materiais:
        tipo = m.get("tipo_material") or "outros"
        grupos.setdefault(tipo, []).append(m)

    for tipo_key, label in TIPO_MATERIAL_LABELS.items():
        lista = grupos.get(tipo_key, [])
        if not lista:
            continue
        st.subheader(label)
        for m in lista:
            with st.expander(
                f"{m['nome']} — R$ {formatar_moeda(float(m['valor']))} "
                f"({TIPO_MEDIDA_LABELS[m['tipo_medida']]})"
            ):
