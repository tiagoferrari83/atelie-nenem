...
def render(tipo_operacao_fixo):
    ...
    with col_srv:
        if servicos:
            # Rótulo do selectbox usa texto simples (não markdown), portanto R$ sem escape é seguro aqui
            servico_opcoes = {
    ...
    if not st.session_state[chave_estado]:
        ...
        for idx_grupo, grupo in enumerate(st.session_state[chave_estado]):
            ...
            with st.container(border=True):
                ...
                with col_a:
                    # Usa formatar_reais() para escapar R\$ e evitar interpretação LaTeX
                    # quando dois valores monetários aparecem na mesma linha de markdown
                    st.markdown(
                        f"**{servico_item['descricao']}** — {servico_item['quantidade']:.2f} x "
                        f"{formatar_reais(servico_item['valor_unitario'])} = "
                        f"{formatar_reais(servico_item['valor_total'])}"
                    )
                with col_b:
                ...
                if grupo["materiais"]:
                    ...
                    for idx_mat, mat in enumerate(grupo["materiais"]):
                        ...
                        with col_m1:
                            # Também usa formatar_reais() para evitar o bug de LaTeX
                            st.markdown(
                                f"　↳ {mat['descricao']} — {mat['quantidade']:.2f} x "
                                f"{formatar_reais(mat['valor_unitario'])} = "
                                f"{formatar_reais(mat['valor_total'])}"
                            )
                        with col_m2:
                ...
                with st.expander("➕ Adicionar matéria-prima a este serviço"):
                    if materiais_cadastrados:
                        ...
                        with col_mat:
                            # Rótulo do selectbox: texto simples, R$ sem escape é seguro
                            material_opcoes = {
                        ...
                        if st.button("Adicionar material", key=f"add_material_{idx_grupo}"):
                            if qtd_material > 0:
                                ...
                                grupo["materiais"].append({
                                    ...
                                    "unidade": TIPO_LABELS_MATERIAL[m["tipo_medida"]],
                                })
                                st.rerun()
                            else:
                                st.error("Informe uma quantidade maior que zero.")
    ...
    if st.button(label_botao, type="primary", disabled=not st.session_state[chave_estado]):
        ...
        # Copia fotos do orçamento de origem para a nova OS (aprovação)
